...
public class EvaTestsMore {
	...
	public static void main(String[] args) {
		...
		Person person1 = new Person.PersonBuilder().accountId(accountIdeetje).name("Eveline").lastName("Noenij")
				.personType("superperson").build();
		...
		Address address1 = new Address.AddressBuilder().personId(personIdeetje).streetName("Straatje").houseNumber(12)
				.postalCode("1231rt").city("supercity").country("CrazyLand").build();
		...
		// int productIdeetje =
		// (productDAO.getProductByName(product.getName())).getProductId();
		...
		OrderLine orderLine = new OrderLine.OrderLineBuilder().order(order2).product(product).numberOfProducts(3)
				.build();
