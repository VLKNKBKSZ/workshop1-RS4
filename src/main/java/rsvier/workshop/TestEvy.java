...
public class TestEvy {
	...
	public static void main(String[] args) {
		...
		// person 1
		/*
		 * Account account1 = new Account("evy@gmail.com", "123456"); Address address1 =
		 * new
		 * Address.AddressBuilder().streetName("Bakemakade").houseNumber(108).postalCode
		 * ("3000ME") .city("Rotterdam").country("Netherlands").build();
		 * accountDao.createAccount(account1); Account accountDatabse1 =
		 * accountDao.getAccount(account1.getEmail(), account1.getPassword()); Person
		 * person1 = (Person) new
		 * Person.PersonBuilder().accountId(accountDatabse1.getAccountId()).personType(
		 * "customer") .name("Evy").lastName("Wetch").middleName("Evyvy").build();
		 * personDao.createPerson(person1); Person person =
		 * personDao.getPersonByLastName(person1.getLastName());
		...
		Person person2 = (Person) new Person.PersonBuilder().accountId(accountDatabse2.getAccountId())
				.personType("employee").name("appie").lastName("pope").build();
		...
		// Product product = new Product.ProductBuilder().name("stole").price(new
		// BigDecimal("100")).stock(5).build();
		...
		// OrderLine.OrderLineBuilder().order(orderDatabse).product(productDatabase).build();
